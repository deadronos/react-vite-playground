...
export default defineConfig(async ({ mode }) => {
  ...
  try {
    ...
    const tailwindccsPlugin =
      tailwaindcssPluginCandidate.default ?? tailwindcss;
    if (typeof tailwindccsPlugin === "function") {
      const result = (
        tailwindccsPlugin as unknown as (
          ...args: unknown[]
        ) => Plugin | Plugin[]
      )();
  ...
  try {
    ...
    if (typeof svgrPlugin === "function") {
      const result = (
        svgrPlugin as unknown as (...args: unknown[]) => Plugin | Plugin[]
      )();
      if (Array.isArray(result)) plugins.push(...result);
      else plugins.push(result);
    }
  ...
  try {
    ...
    if (typeof VitePWA === "function") {
      const result = (
        VitePWA as unknown as (...args: unknown[]) => Plugin | Plugin[]
      )({
